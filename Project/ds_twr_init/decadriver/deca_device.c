...
typedef struct
{
    ...
    uint32      partID;
    uint32      lotID;
    uint8       chan;			// 已经添加的通道，用于累加器的读取
    uint8       longFrames;		// 非标准帧长模式下使用的标记
    uint8       otprev;			// OTP修订号，在初始化期间读取
    uint32      txFCTRL;		// 保存TX_FCTRL寄存器的配置（本地的备份，方便查询）
    uint8       xtrim;			// 从OTP读取的XTAL（外部晶振）的调整值
    uint8       dblbuffon;		// 双缓存区模式下的标志位
    uint32      sysCFGreg;		// 系统配置寄存器的本地副本
    uint16      sleep_mode;		// 用于唤醒时，自动重装载 Used for automatic reloading of LDO tune and microcode at wake-up

    dwt_callback_data_t cdata;	// 回调的数据结构

    uint8       wait4resp;		// 是否等待响应 wait4response was set with last TX start command
    int         prfIndex;
...
static dwt_local_data_t dw1000local ; // 本地静态数据
...
int dwt_initialise(uint16 config)
{
    ...
    dw1000local.dblbuffon = 0; 		// 默认不启动双buff模式
    dw1000local.prfIndex = 0; 		// 16MHz（大概是分频的意思？）
    dw1000local.cdata.aatset = 0; 	// 不打开自动ACK
    ...
    _dwt_enableclocks(FORCE_SYS_XTI); // 注意：将系统时钟设置为XTI——这是确保_dwt_otpread读取的值可靠的必要条件。
 ...
 * @fn dwt_readdevid()
 *
 * @brief 这用于返回DW1000设备的读取设备类型和修订信息 (MP part is 0xDECA0130)
 *
 * input parameters
 *
 * output parameters
 *
 * 返回DW1000的读取值为0xDECA0130
 */
 ...
 *
 * input parameters:
 * @param recordNumber  - 正在访问的寄存器文件或缓冲区的ID
