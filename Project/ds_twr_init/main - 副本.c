...
int main(void)
{
    ...
    /* åˆå§‹åŒ–æ¿è½½ç¡¬ä»¶ï¼ŒåŒ…æ‹¬ä¸²å£ï¼ŒIOå£ï¼ŒSPIç­‰ */
    peripherals_init();

    printf("hello dwm1000!\r\n");

    /* å¤ä½å¹¶åˆå§‹åŒ–DW1000ã€‚ä¸ºäº†åˆå§‹åŒ–DW1000ï¼Œéœ€è¦å æ—¶å°†DW1000çš„å·¥ä½œé¢‘çŽ‡è®¾ç½®ä¸ºæ™¶æŒ¯é¢‘çŽ‡ï¼Œ
	åˆå§‹åŒ–SPIé€ŸçŽ‡åŽå¯ä»¥å¢žåŠ åˆ°æœ€ä½³æ€§èƒ½ */
	
    reset_DW1000(); /* å°†RSTnè½¬åŒ–ä¸ºè¾“å‡ºå¼•è„šæ‹‰ä½Žä¸€æ®µæ—¶é—´ï¼Œå†è½¬æ¢å›žåŽ» */
    spi_set_rate_low();
    ...
    /* é…ç½®DW1000ã€‚ è¯·å‚é˜…ä¸‹é¢çš„æ³¨é‡Š6 */
    dwt_configure(&config);
    dwt_setleds(1);

    /* åº”ç”¨é»˜è®¤çš„å¤©çº¿å»¶è¿Ÿå€¼ã€‚ è¯·å‚é˜…ä¸‹é¢çš„æ³¨é‡Š1 */
